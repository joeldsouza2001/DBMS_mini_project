...
class Tabs:
    ...
    def addRecord(self):
        ...
        if self.table_name=='course':
            ...
            root2.geometry('550x620')
        elif self.table_name=='employee':
            root2.geometry('590x700')
        elif self.table_name=='vehicle':
            root2.geometry('550x580')
        elif self.table_name=='class':
            root2.geometry('550x480')
        else:    
            root2.geometry('580x550')
        ...
        if self.table_name=='vehicle':
            ...
            for i in range(1,len(self.cols1)):
                Label(data_frame,text=f'{self.cols1[i]}',padx=15,pady=20,bg='lightblue').grid(row=i-1,column=0)
                if self.cols2[i]=='mode':
                    pay=StringVar()
                    small_frame=Frame(data_frame,bg='lightblue')
                    Radiobutton(small_frame,text='Cash',value='Cash',variable=pay).grid(row=0,column=0,padx=20)
                    Radiobutton(small_frame,text='Online',value='Online',variable=pay).grid(row=0,column=1,padx=20)
                    small_frame.grid(row=i-1,column=1)
                    self.entries[i]=Entry(data_frame,text=pay)
                else:    
                    self.entries[i]=Entry(data_frame,width=40)
                    self.entries[i].grid(row=i-1,column=1)
        ...
        if self.table_name in ['customer','employee']:
            ...
            for i in range(1,len(self.entries)):
                ...
                if self.entries[i]=='trainer_id':
                    print(self.trains[self.train_cmbx.current()].split()[0])
                    details.append(self.trains[self.train_cmbx.current()].split()[0])
                else:
                    details.append(self.entries[i].get())
    ...
    def deleteRecord(self):
        ...
        if self.table_name=='course':
            ...
            root2.geometry('550x650')
        elif self.table_name=='employee':
            root2.geometry('590x700')
        elif self.table_name=='vehicle':
            root2.geometry('550x600')
        elif self.table_name=='class':
            root2.geometry('550x500')
        else:    
            root2.geometry('580x570')
        root2.title('LEARN2DRIVE')
        self.is_delete=True
        #--------------------frame to enter id --------------------------------
        ...
        try:   
            for i in range(1,len(self.cols2)):
                if self.table_name=='vehicle':   
                    ...
                    c.execute(f"update {self.table_name} set {self.cols2[i]}='{details[i-1]}' where {self.cols2[0]}={id}")
                    conn.commit()
                    c.execute(f"update course set total_class_remaining=total_class_enrolled-total_class_done")
                else:
                    c.execute(f"update {self.table_name} set {self.cols2[i]}='{details[i-1]}' where {self.cols2[0]}={id}")
            conn.commit()
            self.tv.delete(*self.tv.get_children())
